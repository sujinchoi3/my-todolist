...
**작성일**: 2026-02-10 | **버전**: 1.0 | **상태**: Draft
...
| **대상 사용자** | 개인 일정 관리가 필요한 모든 사용자 |
| **제공 형태** | 브라우저 기반 웹 애플리케이션 |
...
- `오늘 날짜 > due_date` 이고 상태가 `pending`인 경우 "마감 기한 경과" 처리
- 마감 기한 경과 항목은 UI에서 시각적으로 구분 (색상/아이콘)
- `is_overdue`는 저장 컬럼이 아닌 조회 시 계산 처리

### 3.5 일정 목록 조회 규칙

- 인증된 사용자는 자신의 할일 전체 목록을 조회할 수 있음
- 기본 정렬: `due_date` 오름차순 (마감일 빠른 순)
- 상태 필터링: `status` 파라미터로 `pending` / `completed` 구분 조회 가능
- 마감 기한 경과 항목은 목록 상단 또는 별도 구분으로 표시
...
| ID | 유스케이스 | 액터 | 설명 |
|----|----------|------|------|
| UC1 | 회원가입 | 비인증 사용자 | 이메일, 이름, 비밀번호 입력 후 계정 생성 |
| UC2 | 로그인 | 비인증 사용자 | 이메일/비밀번호 인증으로 애플리케이션 접근 |
| UC3 | 로그아웃 | 인증된 사용자 | 세션 종료 |
| UC4 | 할일 생성 | 인증된 사용자 | 제목, 설명(선택), 마감일 입력하여 할일 등록 |
| UC5 | 할일 목록 조회 | 인증된 사용자 | 자신의 모든 할일 목록 조회 (마감 기한 경과 구분, 상태 필터 포함) |
| UC6 | 키워드 검색 | 인증된 사용자 | 제목/설명 기준 키워드로 할일 검색 |
| UC7 | 할일 수정 | 인증된 사용자 | 제목, 설명, 마감일 변경 |
| UC8 | 완료 처리 | 인증된 사용자 | 할일 상태를 `completed`로 변경 |
| UC9 | 할일 삭제 | 인증된 사용자 | 특정 할일 삭제 |
...
| 항목 | 요구사항 |
|------|---------|
| **비밀번호 저장** | bcrypt 또는 argon2 해시 알고리즘 적용 |
| **인증 방식** | JWT 기반 토큰 인증 |
